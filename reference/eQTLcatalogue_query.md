...
Determines which datasets to query using `qtl_search`. Uses coordinates
from stored summary stats files (e.g. GWAS) to determine which regions
to query from *eQTL Catalogue*. Each locus file can be stored
separately, or merged together to form one large file with all query
results.
