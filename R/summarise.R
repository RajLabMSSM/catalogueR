...
#
# manhattan_facets <- function(coloc_QTLs,
#                              qtl.thresh = 1e-5,
#                              PP.H4.thresh=.8,
#                              gwas_label="GWAS"){
...
#     geom_point(aes(color=eGene), size=.5, show.legend  = FALSE) +
...
#
#   # Group and melt
#   gwas.qtl.melt <-  setDT(gwas.qtl)[, .(Count = uniqueN(SNP[UCS==T],na.rm  = TRUE)),
...
#                           pattern="*.tsv.gz$", full.names  = TRUE, recursive  = TRUE)
...
#
#   qtl.cols <- grep(".QTL$",colnames(topQTL), value  = TRUE)
...
#   }
#   messager("+ Filtering: p <",qtl_pvalue_thresh)
#   sigQTL <- topQTL %>%
...
#     dplyr::summarise('Lead GWAS SNPs' = n_distinct(SNP[leadSNP], na.rm = TRUE),
#                      "Consensus SNPs" = n_distinct(SNP[Consensus_SNP], na.rm = TRUE),
#                      "UCS SNPs" = n_distinct(SNP[Support>0],na.rm  = TRUE) ) %>%
#     data.table::data.table() %>%
...
#     sigQTL_count$Locus <- factor(sigQTL_count$Locus,  levels = levels(locus_order$Locus), ordered  = TRUE)
#   }
#
...
#   messager("Constructing file names for each GWAS-QTL locus...")
...
#   messager("+",length(top_files),"file names returned.")
...
#     # geom_point(data = subset(topQTL_sig, Consensus_SNP), aes(x=Locus.eGene, y=qtl.ID), color="goldenrod2", show.legend  = TRUE) +
#     # # UCS SNP markers
#     # geom_point(data = subset(topQTL_sig, Support>0), aes(x=Locus.eGene, y=qtl.ID), size=3, shape=1, color="green2", show.legend  = TRUE) +
#     # lead GWAS SNP markers
#     geom_point(data = subset(topQTL_sig, leadSNP), aes(x=Locus.eGene, y=qtl.ID), size=3, shape=1, color="black", show.legend = TRUE) +
...
#   geom_point(size=.1) +
#   geom_hline(yintercept = -log10(5e-8), show.legend  = FALSE, alpha=.5, size=.1) +
