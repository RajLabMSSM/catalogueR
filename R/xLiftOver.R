...
#'
#' \code{xLiftOver} was extracted from the
#' \href{http://xgr.r-forge.r-project.org}{XGR} package.
#' @export
xLiftOver <- function(data.file,
                      format.file = c(
                          "data.frame", "bed", "chr:start-end",
                          "GRanges"
                      ),
                      build.conversion = c(
                          NA, "hg38.to.hg19", "hg19.to.hg38",
    ...
    if (verbose) {
        now <- Sys.time()
        message(sprintf(
            "First, import the files formatted as '%s' (%s) ...",
            format.file, as.character(now)
        ), appendLF = TRUE)
    }
    if (verbose) {
        now <- Sys.time()
    ...
    if (verbose) {
        now <- Sys.time()
        message(sprintf(
            "Second, construct GenomicRanges object (%s) ...",
            as.character(now)
        ), appendLF = TRUE)
    }
    if (format.file == "data.frame") {
        ...
        ind <- suppressWarnings(which(!is.na(as.numeric(data[
            ,
            2
        ])) & !is.na(as.numeric(data[, 3]))))
        data <- data[ind, ]
        dGR <- GenomicRanges::GRanges(seqnames = S4Vectors::Rle(data[
            ,
            1
        ]), ranges = IRanges::IRanges(start = as.numeric(data[
            ,
            2
        ]), end = as.numeric(data[, 3])), strand = S4Vectors::Rle(rep(
            "*",
            nrow(data)
        )))
        ...
        dGR <- GenomicRanges::GRanges(seqnames = S4Vectors::Rle(data[
            ...
            ,
            2
        ]) + 1, end = as.numeric(data[, 3])), strand = S4Vectors::Rle(rep(
            "*",
            nrow(data)
        )))
    ...
    if (verbose) {
        now <- Sys.time()
        message(sprintf(
            "Third, lift intervals between genome builds '%s' (%s) ...",
            build.conversion, as.character(now)
        ), appendLF = TRUE)
    }
    chains <- xRDataLoader(
        RData.customised = "chain", RData.location = RData.location,
    ...
    if (merged) {
        ...
        if (verbose) {
            now <- Sys.time()
            message(sprintf(
                "Finally, keep the first range if multiple found (%s) ...",
                as.character(now)
            ), appendLF = TRUE)
        }
        ...
        ls_df <- lapply(res_ls, function(x) {
            c(
                as.character(unique(x$seqnames))[1], min(x$start),
        ...
        gr <- GenomicRanges::GRanges(seqnames = S4Vectors::Rle(df[
            ,
            1
        ]), ranges = IRanges::IRanges(start = as.numeric(df[
            ,
            2
        ]), end = as.numeric(df[, 3])), strand = S4Vectors::Rle(df[
            ,
            4
        ]))
...
xRDataLoader <- function(RData = c(
                             NA, "GWAS2EF", "GWAS_LD", "IlluminaHumanHT",
                         ...
                         RData.customised = NULL, verbose = TRUE, RData.location = "http://galahad.well.ox.ac.uk/bigdata",
                         guid = NULL) {
    startT <- Sys.time()
    if (verbose) {
        message(paste(c("Start at ", as.character(startT)), collapse = ""),
    ...
    if (!is.null(guid) && nchar(guid) == 5) {
        ...
        if (all(pkgs %in% rownames(utils::installed.packages()))) {
            ...
            if (all(tmp)) {
                prj <- fls <- res <- NULL
                if (all(class(suppressWarnings(try(
                    eval(parse(text = paste0("prj<-osfr::osf_retrieve_node(guid)"))),
                    T
                ))) != "try-error")) {
                    ...
                    if (nrow(fls) > 0) {
                        ...
                        if (length(ind) == 1) {
                            ...
                            eval(parse(text = paste0("res <- fl %>% osfr::osf_download(overwrite = TRUE, path=destfile)")))
    ...
    my_https_downloader <- function(url, method = c(
                                        "auto", "internal",
                                        "wininet", "libcurl", "wget", "curl"
                                    ), quiet = TRUE, mode = c(
                                        "w",
                                        "wb", "a", "ab"
                ...
                if (flag_failed) {
                    ...
                    for (i in 1:length(load_remotes)) {
                        ...
                        if (verbose) {
                            now <- Sys.time()
                            message(sprintf(
                                "Attempt to download from %s (at %s)",
                                load_remote, as.character(now)
                            ), appendLF = TRUE)
                        }
                        res <- my_https_downloader(load_remote, mode = "wb")
                        if (res$flag == TRUE) {
    ...
    if (verbose) {
        ...
        if (!is.null(out)) {
            message(sprintf(
                "'%s' (from %s) has been loaded into the working environment (at %s)",
                RData, load_RData, as.character(now)
            ), appendLF = TRUE)
        } else {
            message(sprintf(
                "'%s' CANNOT be loaded (at %s)",
                RData, as.character(now)
            ), appendLF = TRUE)
        }
    }
    endT <- Sys.time()
