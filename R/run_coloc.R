...
#' @param gwas_sample_size Sample size of the GWAS data 
...
run_coloc <- function(gwas.qtl_paths,
    ...
    qtl_id <- gene.QTL <- SNP.PP.H4 <- NULL;
    qtl.groups <- unique(unlist(lapply(unname(gwas.qtl_paths), parse_gwas.qtl_path)))
        ...
        coloc_qtls <- parallel::mclapply(
            ...
            function(qtl.path,
                ...
                try({
                    ...
                    messager(
                        "++ GWAS =", gwas.locus, "x",
                        formatC(length(unique(qtl.dat$gene.QTL)), big.mark = ","),
                        "eGenes",
                        v = verbose
                    )
                    #### Check column names ####
                    ...
                    qtl.dat <- get_sample_size(
                        subset_DT = qtl.dat,
                        sample_size = .gwas_sample_size,
                        effective_ss = TRUE,
                        verbose = verbose
                    )
