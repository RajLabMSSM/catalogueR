...
#' @param infer_region Infer the range of coordinates to query based on
...
#' @param progress_bar \code{=TRUE} allows progress to be monitored
#'  even when multithreading enabled.
#' Requires R package \pkg{pbmcapply}.
...
#'     sumstats_paths = sumstats_paths[1],
...
eQTL_Catalogue.query <- function(sumstats_paths = NULL,
    ...
    # merge_with_gwas=F; nThread=1; loci_names = basename(dirname(dirname(sumstats_paths))); qtl.id=qtl_datasets; multithread_qtl=T;  multithread_loci=F; quant_method="ge";   split_files=T; multithread_tabix=F; split_files=T;
    ...
    #### Clean ####
    cleanup_tbi(DIR = "./") #.tbi sometimes gets written to pwd
    cleanup_tbi(DIR = dirname(output_dir))
    #### Clock it ####
    query.end <- Sys.time()
